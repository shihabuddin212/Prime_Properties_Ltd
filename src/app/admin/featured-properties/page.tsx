...
import { Plus, Pencil, Trash2, Eye, EyeOff, Loader2, X, Save, Upload, PlusCircle, Trash, FileText, Image as ImageIcon, Film, Star } from 'lucide-react';
...
const EMPTY: Partial<Property> = {
    ...
    floorPlans: '', isFeatured: true, isSpecialOffer: false, published: true, // Default to true for Featured Properties page
    address: '', floors: '', unitsPerFloor: '1', launchDate: '', completionDate: '', brochureUrl: '', mapUrl: '',
};

export default function AdminFeaturedPropertiesPage() {
    ...
    const fetchProperties = async () => {
        ...
        // Filter: Only list the ones marked as Featured and NOT marked as Special Offer
        setProperties(data.filter((p: any) => p.isFeatured && !p.isSpecialOffer));
        setLoading(false);
    };
    ...
    const handleSave = async () => {
        ...
        const payload = {
            ...editing,
            isSpecialOffer: false, // Ensure it's never a Special Offer
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <Star size={20} className="text-luxury-gold fill-luxury-gold/20" /> Homepage Featured Properties
                    </h1>
                    <p className="text-xs text-gray-400 font-light mt-0.5">{properties.length} active featured items shown on home</p>
                </div>
                <button onClick={openCreate}
                    ...
                    <Plus size={14} /> Add Featured Property
                </button>
            </div>
                            ...
                            <tbody className="divide-y divide-gray-50 dark:divide-gray-800/50">
                                {properties.map(p => (
                                    <tr key={p.id} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition">
                                        <td className="px-5 py-3 font-medium text-gray-800 dark:text-gray-200 whitespace-nowrap flex items-center gap-1.5 pt-4 flex-wrap">
                                            {p.title}
                                            <span className="text-[9px] bg-luxury-gold/20 text-luxury-gold px-1.5 py-0.5 rounded font-bold uppercase tracking-wider flex items-center gap-0.5">
                                                <Star size={8} className="fill-luxury-gold" /> FEATURED
                                            </span>
                                            {p.isSpecialOffer && <span className="text-[9px] bg-[#111111]/90 text-white border border-neutral-700/50 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">SPECIAL OFFER</span>}
                                        </td>
                                ...
                                {properties.length === 0 && (
                                    <tr><td colSpan={7} className="px-5 py-8 text-center text-gray-400 font-light">No featured properties yet. Click "Add Featured Property" to create one.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="bg-primary-navy border border-gray-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in scale-in duration-200">
                        <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between sticky top-0 bg-primary-navy z-10">
                            <h2 className="text-sm font-bold text-white uppercase tracking-wider">
                                {editing.id ? 'Edit Featured Property details' : 'Add New Featured Property'}
                            </h2>
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="border border-gray-800/80 rounded-xl p-5 bg-primary-dark/30 space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <input type="text" name="title" value={editing.title ?? ''} onChange={handleTitleChange} placeholder="e.g. The Lighthouse"
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input type="text" name="slug" value={editing.slug ?? ''} onChange={handleChange} placeholder="e.g. the-lighthouse-bashundhara"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase tracking-wider font-semibold text-gray-400">General Location *</label>
                                        <input type="text" name="location" value={editing.location ?? ''} onChange={handleChange} placeholder="e.g. Block M, Bashundhara R/A, Dhaka"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                    <div className="space-y-1 font-mono">
                                        <label className="text-[10px] uppercase tracking-wider font-semibold text-gray-400">Price *</label>
                                        <input type="text" name="price" value={editing.price ?? ''} onChange={handleChange} placeholder="e.g. 5.5 Crore BDT"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="border border-gray-800/80 rounded-xl p-5 bg-primary-dark/30 space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    <div className="space-y-1 col-span-1 sm:col-span-2 md:col-span-3">
                                        ...
                                        <input type="text" name="address" value={editing.address ?? ''} onChange={handleChange} placeholder="e.g. Plot-88, Avenue 4, Block M, Bashundhara R/A, Dhaka"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase tracking-wider font-semibold text-gray-400">Land Area Size *</label>
                                        <input type="text" name="land" value={editing.land ?? ''} onChange={handleChange} placeholder="e.g. 10 Katha"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase tracking-wider font-semibold text-gray-400">Floors count</label>
                                        <input type="text" name="floors" value={editing.floors ?? ''} onChange={handleChange} placeholder="e.g. B+G+12"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                    <div className="space-y-1">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input type="text" name="launchDate" value={editing.launchDate ?? ''} onChange={handleChange} placeholder="e.g. March 2024"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                    <div className="space-y-1 col-span-1 sm:col-span-2">
                                        <label className="text-[10px] uppercase tracking-wider font-semibold text-gray-400">Expected Completion date</label>
                                        <input type="text" name="completionDate" value={editing.completionDate ?? ''} onChange={handleChange} placeholder="e.g. December 2027"
                                            className="w-full bg-primary-dark/60 border border-gray-800 focus:border-luxury-gold/50 rounded-xl px-4 py-2.5 text-xs text-white outline-none transition" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-6 flex-wrap">
                                {[
                                    { label: 'Featured Property (Always checked)', name: 'isFeatured', disabled: true },
